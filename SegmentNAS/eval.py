...
# python SegmentNAS/eval.py SegmentNAS/configs/eval/supernet_base.txt (未验证可行性)
...
def main_worker(gpu, ngpus_per_node, args, cfg):
    ...
    if args.ckpt:
        key = 'model'
        _ckpt = torch.load(open(args.ckpt, "rb"), map_location=torch.device("cpu"))
        logger.info("Successfully load whole ckpt {} from {}".format(args.ckpt, key))
