...
def infer(model, images, **kwargs):
    ...
    pred1, mid_features = model(images, **kwargs)
...
def build_iter_lambda_scheduler(optimizer, *,
                                warmup_iters=500,
                                warmup_ratio=0.001,
                                milestones=(8, 11),
                                ...
                                gamma=0.1):
    """
    返回一个 LambdaLR：前 warmup_iters 线性增，到基础lr；
    之后按 epoch 的 milestones 乘上 gamma 的阶梯衰减（每到一个milestone再乘一次）。
    """
    def lr_lambda(current_iter: int):
        # 1) 线性 warmup
        if current_iter < warmup_iters:
            # 从 warmup_ratio * lr 线性到 1.0 * lr
            return warmup_ratio + (1.0 - warmup_ratio) * (current_iter / float(warmup_iters))

        # 2) 里程碑（按 epoch）衰减
        # 根据当前 iter 推出所在 epoch
        epoch = current_iter // iters_per_epoch
        factor = 1.0
        for m in milestones:
            if epoch >= m:
                factor *= gamma
