...
# -*- coding: utf-8 -*-
"""
MMDet FP32 Latency & Throughput Benchmark
==========================================
Benchmark any MMDetection model (via official config + optional checkpoint)
for FP32 forward-pass latency (bs=1) and throughput (bs=N).

Usage example:
    python bench_mmdet_fp32.py \
...
def build_model(config_path: str, checkpoint_path: str | None, device: torch.device):
    """
    Build mmdet model from config (+ optional checkpoint).
    Tries init_detector first; falls back to Config + MODELS.build.
    """
    from mmengine import Config

    # Register all mmdet modules so that the registry is populated
    try:
    ...
    # Attempt 1: mmdet.apis.init_detector (official high-level API)
    try:
        from mmdet.apis import init_detector
        model = init_detector(config_path, checkpoint_path, device=str(device))
        return model
    except Exception:
        pass
...
def make_data_samples(batch_size: int, height: int, width: int):
    """Construct minimal DetDataSample list for mmdet forward."""
    ...
    return [DetDataSample(metainfo=meta) for _ in range(batch_size)]


def forward_fn(model, inputs, data_samples):
    """
    Run model forward with fallback logic for different mmdet API versions.
    Priority:
      1) model(inputs, data_samples=data_samples, mode='predict')   # mmdet >=3.x
...
def parse_args():
    p = argparse.ArgumentParser(
        description="MMDet FP32 Latency & Throughput Benchmark",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--config", required=True, help="mmdet config file path")
    p.add_argument("--checkpoint", default=None, help="checkpoint file (optional)")
    p.add_argument("--height", type=int, default=800, help="input height")
    p.add_argument("--width", type=int, default=1280, help="input width")
