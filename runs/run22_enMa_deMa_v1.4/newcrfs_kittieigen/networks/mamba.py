...
class MBBlock(nn.Module):
    def __init__(self, C_in, C_out=None, kernel_size=3, stride=1, expansion=6, drop_path=0.,):
        ...
        if expansion != 1:
            ...
            self.op = nn.Sequential(
                nn.Conv2d(C_in * expansion, C_in * expansion, kernel_size, stride=stride, groups=C_in * expansion),
                nn.BatchNorm2d(C_in * expansion),
                nn.ReLU6(inplace=True),
                nn.Conv2d(C_in * expansion, C_out, 1),
                nn.BatchNorm2d(C_out),
...
class MambaVisionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if conv:
            self.blocks = nn.ModuleList([MBBlock(C_in=dim,
