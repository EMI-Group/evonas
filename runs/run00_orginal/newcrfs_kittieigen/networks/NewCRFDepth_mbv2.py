...
from .mobileNetV2 import deepFeatureExtractor_MobileNetV2
...
class NewCRFDepth(nn.Module):
    ...
    def __init__(self, version=None, inv_depth=False, pretrained=None, 
        ...
        if version == 'mobileNetV2':
            self.backbone = deepFeatureExtractor_MobileNetV2()
            embed_dim = 320
            ...
            embed_dim = 768  # 小于512，eval会报错"Expected more than 1 value per channel when training, got input size torch.Size"
            # decoder_cfg = dict(
        ...
        v_dims = [16, 32, 64, embed_dim]  # 必须是crf_dims的一半，严谨一些：v_dims[i]必须是crf_dims[i+1]的四分之一，因为nn.PixelShuffle(2)会降维1/4
        ...
        # self.decoder = PSP(**decoder_cfg)  # 14M参数，将其删除
    ...
    def forward(self, imgs):
        ...
        feats = self.backbone(imgs)  # 27M
        # for x in feats:
        #     print(x.shape)
        # assert False
        # torch.Size([4, 96, 88, 280])
        # torch.Size([4, 192, 44, 140])
        # torch.Size([4, 384, 22, 70])
        # torch.Size([4, 768, 11, 35])

        if self.with_neck:
            feats = self.neck(feats)

        # ppm_out = self.decoder(feats)  # 14M

        e3 = self.crf3(feats[3], feats[3])  # 34M
        e3 = nn.PixelShuffle(2)(e3)
        e2 = self.crf2(feats[2], e3)  # 8M
        ...
        if self.up_mode == 'mask':
            ...
            d1 = self.disp_head1(e0, 4)  # conv + sigmoid, 4为上采样倍率
