...
class NewCRFDepth(nn.Module):
    ...
    def __init__(self, version=None, inv_depth=False, pretrained=None, 
        ...
        self.mba3 = MambaVisionLayer(dim=in_channels[3], depth=depths[0], num_heads=num_heads[0], window_size=window_size[0], mlp_ratio=4, qkv_bias=True, qk_scale=False, drop=0., attn_drop=0., drop_path=drop_path_rate, transformer_blocks=list(range(depths[0]//2+1, depths[0])) if depths[0]%2!=0 else list(range(depths[0]//2, depths[0])))
    ...
    def forward(self, imgs):
        ...
        for i in range(len(feats)):  # 恢复feats结构
            feats[i] = feats[i].permute(0, 2, 1).reshape(Bb, -1, Hh//(2**(i+2)), Ww//(2**(i+2))).contiguous()
        #     print(f'feats[{i}]:{feats[i].shape}')
        # assert False

        # feats[0]:torch.Size([8, 64, 88, 280])
        # feats[1]:torch.Size([8, 128, 44, 140])
        # feats[2]:torch.Size([8, 256, 22, 70])
        # feats[3]:torch.Size([8, 512, 11, 35])
