...
class VSSM(nn.Module):
    ...
    @staticmethod
    def _make_patch_embed_v2(in_chans=3, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False, change=True):
        # if channel first, then Norm and Output are both channel_first
        stride = patch_size // 2 if change else 1
        kernel_size = stride + 1 if change else 3
        padding = 1
        return nn.Sequential(
